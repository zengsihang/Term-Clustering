...
def ratio_cut(terms_list, similarity, ratio):
    ...
    if min(mean_sim0, mean_sim1)/(cut_mean_sim+1e-8)<ratio and len(terms_list)<50:
        clu0 = terms_list
        clu1 = []
...
if __name__ == '__main__':
    ...
    # ratio = 1.5
    # need_cluster_list = [cluster_res[key] for key in need_cluster_list]
    # # test_res = []
    # with Pool(10) as p:
    #     test_res = p.map(re_cluster, need_cluster_list)
    # # for key in need_cluster_list:
    # #     re_cluster_list = re_cluster(list(cluster_res[key]), mode, similarity, ratio)
    # #     test_res.append(re_cluster_list)
    # with open('test_res_par.txt', 'w') as f:
    #     for res in tqdm(test_res):
    #         f.write('----------------------------------------------------\n')
    #         for one_cluster_result in res:
...
#
    ...
    print(len(final_res))
    terms = []
    with open('../result/cluster_res_en.txt', 'w') as f:
